...
const MicroBitImage IMAGE_HAPPY =
...
void onNewLightSensorsData(MicroBitEvent) {
  ...
  uBit.sleep(1); // Prevents an 020 error. 🤷
  char buffer[23];
  snprintf(buffer, 23, "light-sens:%d,%d", lightSensors.getLeftReading(),
           lightSensors.getRightReading());
  uart->send(buffer, ASYNC);
}
...
void onNewLineSensorsData(MicroBitEvent) {
  ...
  uBit.sleep(1); // Prevents an 020 error. 🤷
  char buffer[23];
  snprintf(buffer, 23, "line-sens:%d,%d", lineSensors.getLeftReading(),
           lineSensors.getRightReading());
  uart->send(buffer, ASYNC);
}
...
int main() {
  ...
  uBit.messageBus.listen(MICROBIT_ID_BUTTON_B, MICROBIT_BUTTON_EVT_CLICK,
                         onButtonB);
  uBit.messageBus.listen(MICROBIT_ID_BLE_UART, MICROBIT_BLE_EVENT_SERVICE,
                         onUartEvent);
  // uBit.messageBus.listen(MICROBIT_ID_ACCELEROMETER,
  // MICROBIT_ACCELEROMETER_EVT_DATA_UPDATE, onNewAccelData);
  uBit.messageBus.listen(GIGGLEBOT_ID_BATTERY, GIGGLEBOT_BATTERY_EVT_UPDATE,
                         onNewBatteryData);
  uBit.messageBus.listen(GIGGLEBOT_ID_LIGHT_SENSORS,
                         GIGGLEBOT_LIGHT_SENSORS_EVT_UPDATE,
                         onNewLightSensorsData);
  uBit.messageBus.listen(GIGGLEBOT_ID_LINE_SENSORS,
                         GIGGLEBOT_LINE_SENSORS_EVT_UPDATE,
                         onNewLineSensorsData);
